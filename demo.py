...
def SRCNN_sr(sess, SRCNN, input, scale):
    ...
    srcnn_y_ch = sess.run(SRCNN.output, feed_dict={SRCNN.X:np.expand_dims([low_rs], axis=-1)})[0]
    ycrcb[:, :, 0] = (srcnn_y_ch[:,:,0] * 255)
    rgb = np.clip(cv2.cvtColor(ycrcb, cv2.COLOR_YCR_CB2RGB), 0, 255)
...
if __name__ == '__main__':
    ...
    original_list = np.array([input, bicubic_output, SRCNN_output])
